...
def override_require_admin():
    return RequestIdentity(
        user_id="test-admin", is_admin=True, auth_provider="test"
    )  # noqa: E501
...
def test_eligibility_rule_crud(admin_client: TestClient):
    ...
    response = admin_client.put(
        f"/api/admin/eligibility-rules/{rule_id}",
        json={
...
def test_deadline_crud(admin_client: TestClient):
    ...
    response = admin_client.put(
        f"/api/admin/deadlines/{deadline_id}",
        json={
